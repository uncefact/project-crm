...
This workstream is intended to answer the “How” for data exchange and the sustainability working group is answering the “What” data to exchange.

This document also provides the context and guidelines for the technical deliverables for the CRM pilots such as credential schema and traceability vocabularies to drive the adoption of traceability standards that enable verifiable claims.

### Audience 

The primary audience for this document includes the following:
...
The focus of this document is traceability and interoperability of the critical raw materials industry.  In addition, there is parallel work going on in Recommendation 49 about transparency at the scale that this document will inform and we will seek guidance from. It is important that traceability interoperability works for the CRM industry in the same way as it works in adjacent and upstream industries.
...
This section should define a specific data model of the simplest possible EPCIS event structure and vocabulary that is sufficient for end-to-end value chain traceability. Less is more because simple low-cost implementation is key. Draw upon existing UN/CEFACT vocabulary.
...
This section should define preferred identity schemes for each type of identifier.
...
* Peer-to-peer - peer-to-peer trust networks (community members keeping each other honest)
...
A design solution to verifiably link the digital data to the physical shipment.

* Finding ESG data from product ID using Digital Link resolvers
* Discovering the right resolver service

### Provenance verification 

List known mechanisms for provenance verification

* Chemical / Isotopic fingerprints
